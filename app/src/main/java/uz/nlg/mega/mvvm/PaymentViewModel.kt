...
import uz.nlg.mega.utils.PaymentPriceError
...
@Inject constructor(
    ...
    private val _payed = mutableStateOf(false)
    val isPayed = _payed
    ...
    fun returnCredits(credit: Credit) = viewModelScope.launch {
        ...
        if (credit.totalPrice <= 0) {
            _error.value = PaymentPriceError
            return@launch
        }

        Log.d("TAG", "returnCredits: $credit")
        ...
        try {
            ...
            while (isStillCalling) {
                ...
                val handler =
                    NetworkHandler(repository.returnCredits(credit), ErrorResponse::class.java)
